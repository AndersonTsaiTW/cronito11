...
export default function MapView() {
  ...
  useEffect(() => {
    ...
    shelterMarkersRef.current = shelters.map((shelter) => {
      ...
      const el = document.createElement('button')
      el.type = 'button'
      el.className = `shelter-marker${isSelected ? ' shelter-marker--selected' : ''}${isRecommended ? ' shelter-marker--recommended' : ''}`
      el.style.setProperty('--marker-color', getShelterStatusColor(shelter))
      el.style.setProperty('--marker-size', `${size}px`)
      el.style.width = `${size}px`
      el.style.height = `${size}px`
      el.title = `${shelter.name} - ${shelter.occupancy}/${shelter.capacity} occupied${isRecommended ? ' (Recommended)' : ''}`
  ...
  }, [mapReady, shelters, selectedShelterId, selectShelter, recommendedShelterId])
