...
import type { ForecastOffset, LayerId } from '../types'
...
const LAYERS: { id: LayerId; label: string; color: string; info?: string }[] = [
  ...
  { id: 'older_adults', label: 'Older Adults 65+', color: 'bg-pink-500' },
  {
    ...
    label: 'Forecast Temp',
    color: 'bg-red-400',
    info: 'Official forecast temperatures from ECCC MSC GeoMet (citypageweather-realtime). Covers Toronto and surrounding GTA cities. Falls back to mock data if the API is unavailable.',
  },
]

const OFFSET_OPTIONS: { value: ForecastOffset; label: string }[] = [
  { value: 'current', label: 'Now' },
  { value: 'today_high', label: 'Today High' },
  { value: 'tomorrow_high', label: 'Tomorrow High' },
]

export default function LayerControls() {
  const { visibleLayers, toggleLayer, forecastOffset, setForecastOffset } = useAppStore()
      ...
      <div className="flex flex-col gap-1">
        {LAYERS.map(({ id, label, color, info }) => (
          <div key={id}>
            ...
            {/* Forecast offset selector, shown only when forecast_temperature is active */}
            {id === 'forecast_temperature' && visibleLayers.forecast_temperature && (
              <div className="flex gap-1 mt-1 ml-2">
                {OFFSET_OPTIONS.map(({ value, label: optLabel }) => (
                  <button
                    key={value}
                    onClick={() => setForecastOffset(value)}
